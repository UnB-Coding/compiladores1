# Entendendo o Projeto do Interpretador
...
O **interpretador** é o tradutor. Ele lê as instruções que você escreveu na linguagem de programação e as executa diretamente para que o computador realize as tarefas.

Neste projeto, estamos construindo o nosso próprio interpretador!

---

## 2. Como o nosso Interpretador funciona? (O Passo a Passo)
...
Se a frase está correta, o interpretador a transforma em um "mapa mental" ou um "diagrama em árvore", chamado de AST. 
Isso ajuda o interpretador a entender a ordem correta das operações. Por exemplo, na conta `2 + 3 * 4`, a árvore garante que a multiplicação (`3 * 4`) seja resolvida antes da adição, seguindo as regras da matemática.

### Passo 4: A Tabela de Símbolos
**Onde encontrar no projeto:** Pasta `symbol_table/`

Imagine a Tabela de Símbolos como a "memória de curto prazo" ou o caderninho de anotações do interpretador. 
Quando o programador cria uma variável (por exemplo, guarda o valor `10` na variável `idade`), o interpretador anota na Tabela de Símbolos:
- Nome: `idade`
- Tipo: `inteiro` (int)
- Valor: `10`

Assim, se mais tarde o programa pedir para imprimir a `idade`, o interpretador olha no caderninho e sabe que o valor é `10`. Nosso projeto suporta números inteiros (`int`), números com vírgula (`float`), letras (`char`) e verdadeiro/falso (`bool`).
...
Usamos a linguagem Python (através de uma ferramenta chamada `pytest`) para enviar esses arquivos para o nosso interpretador e verificar se ele acerta as respostas. Se ele errar, sabemos que temos um bug (erro) para corrigir.
...
Um interpretador é feito de vários pequenos arquivos de código. O arquivo `Makefile` é como se fosse um manual de instruções para uma fábrica de montagem. Quando dizemos ao computador para "rodar o make", ele lê o `Makefile` e junta todas as peças (`scanner.l`, `parser.y`, `ast.c`, etc.) para criar o programa final (o interpretador pronto para uso). Os arquivos finais são guardados na pasta `build/`.

---

## 5. Funcionalidades Implementadas (O que a nossa linguagem faz?)

Aqui estão as funcionalidades que o nosso interpretador suporta, como elas funcionam nos bastidores (back-end) e como os arquivos interagem para torná-las possíveis:

### Tipos de Dados Suportados
- **O que faz:** O interpretador entende números inteiros (`int`), números com vírgula (`float`), caracteres únicos (`char` - ex: 'a') e valores verdadeiro/falso (`bool`).
...
- `symbol_table/`: O código responsável pela "memória" do interpretador (variáveis e valores).
...
- `build/`: Pasta gerada automaticamente. É para onde vai o interpretador depois de "montado".
...
- **Nosso Projeto:** Tecnicamente, o nosso interpretador funciona como um **Interpretador Baseado em AST**. Ele lê o código, monta a Árvore Sintática (`parser.y`) e **executa as instruções imediatamente para fins de testagem**, percorrendo a árvore através da função `eval_ast()` no arquivo `ast.c`. Ele não gera um arquivo binário final do código que o usuário escreveu.
