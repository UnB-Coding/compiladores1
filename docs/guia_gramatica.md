...
Durante a execução da IR (`ir_exec()`), o interpretador resolve as incompatibilidades de tipos de dados de acordo com as seguintes regras semânticas:
...
* **Redeclaração:** O interpretador impede a criação de variáveis com o mesmo identificador. Tentativas de redeclaração (ex: `int a; int a;`) são detectadas pela análise semântica (`analyze_ast()`) e abortam o programa **antes** da execução, com código de falha.
* **Variável não declarada:** O uso de qualquer variável em expressões ou atribuições sem declaração prévia é igualmente detectado na fase semântica, encerrando o processo com erro antes de gerar ou executar a IR.
...
* **Coerção de Tipo em Tempo de Execução:** A *coerção* de tipo (promoções e truncamentos) ocorre em tempo de execução, durante a interpretação da IR (`ir_exec()`), e não estaticamente. A *verificação* de erros como uso de variável não declarada, redeclaração e divisão por zero literal, por outro lado, é feita estaticamente pela fase de análise semântica (`analyze_ast()`), **antes** da geração de IR — um programa com esses erros é rejeitado sem chegar a executar.
